...
这个点确实容易被忽略。\textbf{新开会话本身不是斜杠命令。}最直接的做法是退出当前交互，再重新运行：

\begin{lstlisting}[style=cli]
codex
\end{lstlisting}

如果你不是想“全新开始”，而是想基于旧上下文继续分叉或恢复，那再考虑：
\begin{itemize}
  \item \texttt{codex resume --last}：继续最近一次会话；
  \item \texttt{codex fork --last}：从最近一次会话分出一条新分支。
\end{itemize}
...
\subsection{Q5：它改完代码后，我最少要做什么检查？}
