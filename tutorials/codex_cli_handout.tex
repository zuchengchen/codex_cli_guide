...
\begin{tipbox}
这两个变量名别混：上面这条命令是在“登录”时从 \texttt{OPENAI\_API\_KEY} 读入 key；到了 \texttt{codex exec} 和 CI 场景，官方更常用的是 \texttt{CODEX\_API\_KEY}。
\end{tipbox}
...
\begin{itemize}
  ...
  \item 想从 shell 里直接接着最近一次聊：\texttt{codex resume --last}
\end{itemize}

\subsection{能在多个会话间切换吗？}

可以，但更像“恢复某个已保存会话”，不是标签页式常驻切换。日常最常用的是 \texttt{/resume}、\texttt{/fork}，以及 shell 里的 \texttt{codex resume}。
...
如果你只做最低限度检查，至少做三件事：看 \texttt{/diff}、跑相关测试或 lint、自己验证关键路径。时间允许的话，再补一次 \texttt{/review} 会更稳。
