...
/diff & 查看改动 & 改完先审一眼 \\
/plan & 先出计划 & 大任务先计划再动手 \\
/agent & 查看或切换 agent 线程 & 用了 subagent 后看并行线程和细节 \\
...
\textbf{最常用的位置：}
\begin{itemize}
  \item 用户级：\texttt{\textasciitilde/.codex/config.toml}
  \item 项目级：\texttt{.codex/config.toml}
\end{itemize}

\textbf{最值得先认的几类信息：}
\begin{itemize}
  \item 顶层：\texttt{model}、\texttt{approval\_policy}、\texttt{sandbox\_mode}、\texttt{web\_search}
  \item \texttt{[profiles.<name>]}：常用模式
  \item \texttt{[features]}：功能开关
  \item \texttt{[agents]}：subagent 并发和层级
\end{itemize}
...
\begin{tipbox}
当前版本里 \texttt{multi\_agent} 通常默认就是开的。多数时候不是“没开”，而是你没有明确要求它去 spawn subagent。
\end{tipbox}
...
\subsection{subagent 怎么上手}

\begin{lstlisting}[style=prompt]
请 spawn 一个 subagent 负责一个检查点：
1. 安全
2. 真实 bug
3. 测试遗漏
等它们都完成后，再统一总结。
...
\texttt{/agent} 适合看当前有哪些 agent 线程，也方便切过去看细节。

\subsection{怎么开多个 subagent}
...
\texttt{max\_threads} 控同时并行多少个；\texttt{max\_depth} 控能不能继续往下再 spawn。大多数时候先调线程数，不要急着加深层级。

\subsection{自定义 subagent 放哪里}

\begin{itemize}
  \item 个人全局：\texttt{\textasciitilde/.codex/agents/}
  \item 当前项目：\texttt{.codex/agents/}
\end{itemize}

最小自定义文件至少写：\texttt{name}、\texttt{description}、\texttt{developer\_instructions}。
...
\begin{enumerate}
  ...
  \item 想让它并行干活，得明确要求 spawn subagent，并先控住 \texttt{max\_threads}。
\end{enumerate}
\end{tcolorbox}
